...
console.log("görev-1", fenomenler[1].profile);
...
console.log("görev-2.2" , fenomenler[6].profile);
...
function indekseGoreFenomen(fenomenler, indeks) {
  const mess = 
  indeks + '.indekste bulunan fenomen:'+ fenomenler[indeks].profile;
  return mess
}
...
function fenomenSil(fenomenler, indeks) {
   const newArr = [...fenomenler];
   newArr.splice(indeks, 1);
   return newArr;
}
...
function fenomenGonderimSayisi(fenomenler, profile1){
  const fenomen = fenomenler.find(fenomen=>fenomen.profile === profile1);
  if (fenomen) {
    return fenomen.posts.toString();
  } else {
